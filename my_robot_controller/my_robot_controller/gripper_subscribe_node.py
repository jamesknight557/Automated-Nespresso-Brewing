...
class RobotiqGripper( mm.Instrument ):
    ...
    def __init__(self, portname, slaveaddress=9):
        ...
        mm.Instrument.__init__(self, ser, slaveaddress=9)
        self.debug=True
...
class MMSubscriber(Node):
    def __init__(self):
        super().__init__('mm_subscriber')
        self.subscription = self.create_subscription(
            Float64,    # Message type
            'mm',       # Topic
            self.mm_callback,  # Callback function
            10          # Queue size
        )

    def mm_callback(self, msg):
        self.mm_value = int(msg.data)  # Convert float to int and store
        self.get_logger().info(f'Received and stored as integer: {self.mm_value}')


def main(args=None):
    rclpy.init(args=args)
    subscriber = MMSubscriber()
    ...
    while True:
        ...
        if oldgoalmm != subscriber.mm_value:
            grip.goTomm(subscriber.mm_value,100,100)
            oldgoalmm = subscriber.mm_value
        else:
            pass
